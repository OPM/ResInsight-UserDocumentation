...
Before writing or editing a script, read the existing example scripts in `docs/rips/PythonExamples/` that are most relevant to the task. Use `Glob` to discover example files and `Read` to study the patterns they use.
...
Browse `docs/rips/PythonExamples/` for reference scripts covering a wide range of workflows. The sub-directories group examples by topic:
...
Reference: `polygon_grid_region.py`, `polygon_region_from_project.py`, `create_polygon.py`.
