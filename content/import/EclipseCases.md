...
ResInsight supports the following type of reservoir simulation input data:
...
- ROFF grid models from _`*.ROFF`_ and _`*.ROFFASC`_ files.

## Simulation Results
ResInsight offers several ways to import reservoir simulation grid files. Use **File->Import->Import Grid Models** to open the grid model import dialog. Select the appropriate grid model type from the file type list:

- Binary Grid Models (`*.GRID` `*.EGRID`)
- Text Grid Models (`*.GRDECL`)
- Roff Grid Models (`*.ROFF` `*.ROFFASC`)

Select one or more files after choosing the grid model type. The dialog also replaces the separate commands previously used to import binary, text, and ROFF grid models.

{{% notice note %}}
Selecting multiple _`*.GRDECL`_ files imports the grid and its properties in a single operation.
{{% /notice %}}

- **Import Grid Models Recursively**: Brings up the recursive file selection dialog. This dialog is described in detail on the [Summary Plots page]({{% relref "ensemblefiledialog" %}}#ensemble-file-import).
- **Import Grid Model (Time Step Filtered)**: See [description](#time-step-filtered-eclipse-result)
...
Select **File->Import->More-> {{< image-in-text src="images/getting-started/Case24x24.png" >}} Import Grid Model (Time Step Filtered)** and select an _`*.EGRID`_ or _`*.GRID`_ simulation file for import. A dialog will appear.
...
If a binary or text reservoir simulation case is loaded, additional properties can be appended to this binary case. Make sure that the number of values in the GRDECL file matches the total number of cells in the binary case.
