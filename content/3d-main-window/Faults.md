...
If the `*.DATA file` is not found, the folder is searched for `*.IN` file. This file is parsed based on the file structure of a **PFLOTRAN** simulation. Identified faults will be imported similar to the import from `*.DATA` files.
