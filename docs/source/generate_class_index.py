...
* ``EnumClasses.rst`` - reference page for the ``StrEnum`` parameter
  types, one ``autoclass`` block per enum.
...
# Intro text for the enum reference page. The ``.. _EnumClasses:`` label
# lets other pages cross-reference it.
ENUM_INTRO = """\
.. This file is auto-generated by generate_class_index.py - do not edit manually.
...
def render_enum_page(enum_names):
    """Render the enum reference page: one autoclass block per enum."""
    parts = [ENUM_INTRO.rstrip(), ""]
    for name in enum_names:
        parts.append(f".. autoclass:: rips.{name}")
...
def main():
    ...
    parser.add_argument(
        "--enum-output",
        default=os.path.join(script_dir, "EnumClasses.rst"),
        help="Enum reference RST file (default: EnumClasses.rst next to this script).",
    )
    ...
    enum_names = discover_enums()
    with open(args.enum_output, "w", encoding="utf-8") as handle:
        handle.write(render_enum_page(enum_names))
    ...
    print(f"Wrote enum page {args.enum_output} ({len(enum_names)} enums)")
